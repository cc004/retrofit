...
def gen(ecp, pattern):
    ...
    '''
    generate(f'hwlp_pattern{pattern["_id"]}_ecp{ecp}', ['retrofit_clwl', 'start_gap', f'ecp{ecp}'], {
        'row_rand.high': 32,
        'row_rand.low': 32,
        'row_rand.times': '1',
        'row_rand.percent': 0.5,
...
for ecp in [0, 1, 2, 3]:
    for cov in [0.05, 0.15, 0.2, 0.25, 0.3, 0.25, 0.35]:
        pattern = pattern3
        for i in [1,2,3,4]:
            generate(f'arsrs{i}_pattern{pattern["_id"]}_ecp{ecp}_cov{cov}', ['ars', f'ecp{ecp}', 'row_sparing'], {
                'row_rand.high': 10,
                'row_rand.low': 54,
                'row_rand.times': 1,
                'row_rand.percent': 1,
        ...
        generate(f'ars_pattern{pattern["_id"]}_ecp{ecp}_cov{cov}', ['ars', f'ecp{ecp}'], {
            'row_rand.high': 10,
            'row_rand.low': 54,
            'row_rand.times': 1,
            'row_rand.percent': 1,
...
def gen(gap, ecp, cov, percent, flags = [], prefix = ''):
    ...
    generate(f'{prefix}new_arsrs_{percent}_{gap}gap_ecp{ecp}_cov{cov}', [f'ecp{ecp}', 'ars', f'row_sparing'] + flags, {
        'row_rand.high': int(64 * percent),
        'row_rand.low': 64 - int(64 * percent),
        'row_rand.times': '1',
        'row_rand.percent': 1 - percent,
...
for ecp in [0, 1, 2, 3]:
    for gap in [2, 4,8]:
        for fault in [0]:
            '''
            generate(f'arsrs_startfault{fault}_gap{gap}_ecp{ecp}', ['ars', 'row_sparing', f'ecp{ecp}'], {
                'row_rand.high': 10,
                'row_rand.low': 54,
                'row_rand.times': 1,
                'row_rand.percent': 1,
...
'''            
    ...
    'row_rand.percent': 1,
                ...
                generate(f'column_wear_leveling2+retrofit_pattern{paatern["_id"]}_startfault{fault}_gap{gap}_ecp{ecp}', ['retrofit', 'retrofit_clwl', f'ecp{ecp}'], {
    'row_rand.high': 10,
    'row_rand.low': 54,
    'row_rand.times': 1,
    'row_rand.percent': 1,
                ...
                generate(f'column_wear_leveling2+retrofit_gps_pattern{paatern["_id"]}_startfault{fault}_gap{gap}_ecp{ecp}', ['retrofit', 'retrofit_global_pointer', 'retrofit_clwl', f'ecp{ecp}'], {
    'row_rand.high': 10,
    'row_rand.low': 54,
    'row_rand.times': 1,
    'row_rand.percent': 1,
