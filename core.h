...
#if defined(counter_cw) || defined(horizontal_wear_leveling) || defined(hwl)
    int group_bits = 64;
#endif

    int extra_col = 0;

#if defined(ilf) || defined(byte_rotation) || defined(counter_cw)
...
int64_t emulate(float** endurance_t, dataset data)
{
...
#ifdef counter_cw
    int col_counter[rows + cfg.extra_row];
    for (int i = 0; i < rows + cfg.extra_row; ++i) col_counter[i] = 0;
#endif
...
#ifdef cluster
    bool reduced[rows + cfg.extra_row][cols + cfg.extra_col];
#endif
    ...
    for (int aaa = 0; aaa < data.count; ++aaa)
    {
...
#if defined(retrofit) && defined(dsp)
        if (space.size() == 0 && error.size() == 0)
        {
            p.row = rand() % rows;
        }
#endif
        ...
        if (prev_not_fault && now_fault)
        {
            ...
            {
#if defined(dsp)
                auto base = lp.row >> cfg.dsp_group_bit << cfg.dsp_group_bit;
                int total = 0;
                for (int i = 0; i < (1 << cfg.dsp_group_bit); ++i)
                    total += fault_count[row_lut[base + i]] >= cfg.aux ? fault_count[row_lut[base + i]] - cfg.aux : -cfg.dsp_pointer_mapping[fault_count[row_lut[base + i]]];
#if defined(retrofit_global_pointer)
                if (total > full.size() * 26)
#else
                if (total > 0)
#endif
                {
...
// For Counter CW, whenever one row reach the swap interval, cols are swapped in groups (8 bits per group usually).
        ...
        {
            float temp[cfg.group_bits];
            for (int i = 0; i < cfg.group_bits; ++i) temp[i] = endurance[p.row][i];
            for (int i = 0; i < cols - cfg.group_bits; ++i) endurance[p.row][i] = endurance[p.row][i + cfg.group_bits] - cfg.swap_endurance;
            for (int i = 0; i < cfg.group_bits; ++i) endurance[p.row][cols - cfg.group_bits + i] = temp[i] - cfg.swap_endurance;
            col_counter[p.row] = 0;
        }
#endif
    }
...
#ifdef dsp
    ::cfg.dsp_group_bit = 1;
    ::cfg.dsp_pointer_mapping[0] = 3;
    ::cfg.dsp_pointer_mapping[1] = 2;
    ::cfg.dsp_pointer_mapping[2] = 1;
    ::cfg.dsp_pointer_mapping[3] = 0;
#endif
#endif
...
#ifdef dsp
    ::cfg.dsp_group_bit = 3;
    ::cfg.dsp_pointer_mapping[0] = 2;
    ::cfg.dsp_pointer_mapping[1] = 1;
    ::cfg.dsp_pointer_mapping[2] = 0;
#endif
#endif
...
#ifdef dsp
    ::cfg.dsp_group_bit = 6;
    ::cfg.dsp_pointer_mapping[0] = 1;
    ::cfg.dsp_pointer_mapping[1] = 0;
#endif
#endif
